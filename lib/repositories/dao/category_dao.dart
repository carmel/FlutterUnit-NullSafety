...
class CategoryDao {
  ...
  Future<int> update(CategoryPo widget) async {
    ...
    return await db!.transaction((tran) async => await tran.rawUpdate(updateSql, [
          widget.name,
          widget.color,
          widget.info,
          widget.priority,
  ...
  Future<int> removeWidget(int? categoryId, int? widgetId) async {
    ...
    return await db!.transaction((tran) async => await tran.rawInsert(deleteSql, [
          categoryId,
          widgetId,
        ]));
  }
  ...
  Future<void> toggleCollect(int? categoryId, int? widgetId) async {
    if (await (existWidgetInCollect(categoryId, widgetId) as FutureOr<bool>)) {
      //已存在: 移除
